...
# 数据集类别名
names = ['swimming', 'tread_water', 'drowning']  # class names

# yolov5s网络中的三个detect层
target_layers = ['model_19_cv3_act', 'model_22_cv3_act', 'model_25_cv3_act']

# Arguments
parser = argparse.ArgumentParser()
parser.add_argument('--model-path', type=str, default="runs/train/exp/weights/best.pt", help='Path to the model')
parser.add_argument('--img-path', type=str, default='../grad_cam_images/', help='input image path')
parser.add_argument('--output-dir', type=str, default='runs/gradcam_outputs/', help='output dir')
...
def main(img_path):
    ...
    save_path = str(increment_path((f'{args.output_dir}{imgae_name[:-4]}'), exist_ok=args.exist_ok))
